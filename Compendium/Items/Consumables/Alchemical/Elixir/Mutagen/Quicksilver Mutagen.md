---
tags:
- item/consumable
- trait/item/consumable
- trait/item/alchemical/elixir
- trait/item/alchemical/mutagen
- trait/effect/polymorph

---
# Quicksilver Mutagen

[Consumable](Consumable.md "Item Trait") [Alchemical](Alchemical.md "Item Trait") [Elixir](Elixir.md "Item Trait") [Mutagen](Mutagen.md "Item Trait") [Polymorph](Polymorph.md "Effect Trait")
 
**Bulk:** L
**Hand Usage:** Held in 1 hand
**Activate:** [](#Actions "Single Action") (manipulate)

You become swifter and nimbler but also fragile.
  
- **Benefit:** You gain an item bonus to Acrobatics checks, Stealth checks, Thievery checks, Reflex saves, and Dexterity-based attack rolls, and you gain the listed status bonus to your Speed.  
- **Drawback:** You take damage equal to twice your level; you can't recover Hit Points lost in this way by any means while the mutagen lasts. You take a –2 penalty to Fortitude saves.

| **Level** | **Item** | **Price** | Effect                     |
| --------- | -------- | --------- | -------------------------- |
| 1         | Lesser   | 4 gp      | The bonus to rolls is +1, the bonus to Speed is +5 feet, and the duration is 1 minute. |
| 3         | Moderate | 12 gp     | The bonus to rolls is +2, the bonus to Speed is +10 feet, and the duration is 10 minutes. |
| 11        | Greater  | 300 gp    | The bonus to rolls is +3, the bonus to Speed is +15 feet, and the duration is 1 hour. |
| 17        | Major    | 3,000 gp    | The bonus to rolls is +4, the bonus to Speed is +20 feet, and the duration is 1 hour. |
